...
const INVESTMENT_ALLOWED_NAMESPACES = new Set(['meta', 'host', 'investment']);

function sanitizeInvestmentLocale(messages: LocaleMessages): LocaleMessages {
  return Object.fromEntries(
    Object.entries(messages).filter(([key]) => INVESTMENT_ALLOWED_NAMESPACES.has(key))
  ) as LocaleMessages;
}
...
for (const module of Object.values(localeModules)) {
  ...
  const messages = sanitizeInvestmentLocale(rawMessages);
