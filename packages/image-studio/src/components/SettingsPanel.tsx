...
import { buildOllamaUrl, normalizeOllamaBaseUrl } from '@image-studio/utils/ollama';
...
export function SettingsPanel() {
  ...
  const fetchOllamaModels = useCallback(async () => {
    ...
    try {
      ...
      const cacheKey = `ollama-models-cache-${normalizedBaseUrl}`;
      ...
      const apiUrl = buildOllamaUrl(baseUrl, '/api/tags');
  ...
  const handleSave = useCallback(async () => {
    setStatus(null);
    const normalizedBaseUrl = provider === 'ollama' ? normalizeOllamaBaseUrl(baseUrl) : baseUrl;
