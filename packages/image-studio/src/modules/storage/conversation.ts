...
async function ensureConversation(taskId: string): Promise<TaskConversationData> {
  const record = await db.taskConversations.get(taskId);
  ...
  await db.taskConversations.put({
    conversationId: taskId,
    taskId,
    imageHash: task?.imageHash,
    imageCode: task?.imageCode,
    conversations,
    createdAt: Date.now(),
    updatedAt: Date.now()
  });
...
export async function addMessageToThread(
  ...
  if (!record.conversations) {
    record.conversations = initializeConversation();
  }

  // 如果是主线程，直接修改
  if (threadId === record.conversations.defaultThread.threadId) {
    record.conversations.defaultThread.messages.push(newMessage);
    record.conversations.defaultThread.updatedAt = Date.now();
  } else if (record.conversations.additionalThreads) {
    // [预留] 其他线程支持
    const thread = record.conversations.additionalThreads.find((t) => t.threadId === threadId);
    if (thread) {
  ...
  await db.taskConversations.update(taskId, {
...
export async function updateConversationSummary(taskId: string, summary: string): Promise<void> {
  ...
  record.conversations.conversationSummary = summary;
  await db.taskConversations.update(taskId, {
    conversations: record.conversations,
    updatedAt: Date.now()
  });
