...
import { resolveAgentPrompt } from '@/config/agents';
...
import {
  ...
  getLanguageDisplayName as getLanguageDisplayNameFromConfig,
  isSupportedLanguage,
  normalizeLanguage
...
function validateLanguage(language: string | undefined): Language {
  ...
  if (language && normalized !== language) {
    console.warn(`[PromptAssemble] Invalid language '${language}', defaulting to '${normalized}'`);
  }
  return normalized;
}
...
export function assemblePrompt(
  ...
  const agentPrompt = resolveAgentPrompt(agent, validLanguage);
...
export function getLanguageDisplayName(language: Language): string {
  return getLanguageDisplayNameFromConfig(language);
}
